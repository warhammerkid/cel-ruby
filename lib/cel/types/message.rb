  ...
  module Types
    ...
    class Message
      ...
      def field_set?(key)
        ...
        fd_label = fd.label
        return Cel::Bool.new(fd.get(@message).size != 0) if fd_label == :repeated # rubocop:disable Style/ZeroLengthPredicate

        # Some fields have special presence checks. The "has_FIELD?" method
        # returns either false or 0 for true.
        has_method = :"has_#{key}?"
        return Cel::Bool.new(@message.public_send(has_method) != false) if @message.respond_to?(has_method)
      ...
      def self.unwrap_well_known(message)
        ...
        when Google::Protobuf::ListValue
          ...
          Cel::List.new(elements)
        when Google::Protobuf::Struct
          hash = message.fields.to_a.to_h { |k, v| [Cel::String.new(k), unwrap_proto_value(v)] }
          Cel::Map.new(hash)
        ...
        when Google::Protobuf::Duration
          Cel::Duration.new(seconds: message.seconds, nanos: message.nanos)
        end
      end
      ...
      def self.unwrap_proto_value(value)
        case value.kind
        when nil, :null_value then Cel::Null.new
        when :number_value then Cel::Number.new(:double, value.number_value)
        when :string_value then Cel::String.new(value.string_value)
        when :bool_value then Cel::Bool.new(value.bool_value)
